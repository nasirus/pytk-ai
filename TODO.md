...
- 2026-04-05: Completed `ADD-FILTER-PLAN.md` Phase 6 by adding a dedicated GitHub/API filter module for `gh pr list`/`view`, `gh issue list`, `gh run list`, `curl`, and `wget`; narrowing planner and rewrite rules to the intended GitHub command forms; and adding focused regressions for compact success output and raw-failure preservation.
- 2026-04-05: Fixed Phase 6 regressions by keeping successful non-stdout `curl` download workflows on the raw path, allowing `gh pr view` titles that start with `[` while still rejecting real JSON payloads, and recognizing `wget --output-document=<file>` when summarizing successful downloads.
- 2026-04-05: Closed the remaining `ADD-FILTER-PLAN.md` cross-cutting work by adding shared `FilterMetrics` / `FilterPolicy` metadata to filtered results, recording usage mode without splitting hook vs interactive behavior, extending the lab benchmark with deterministic per-family filter evidence, and marking the plan complete.
- 2026-04-05: Optimized bare `git status` by executing porcelain v1 with branch metadata, summarizing status buckets into denser branch/staged/modified/untracked lines, and adding planner, filter, and live regression coverage for the rewritten execution path.
